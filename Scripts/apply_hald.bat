...
@for %%f in (%*) DO (
    ...
    call %IMC%  "%%f"  -gamma 1.0  %HALD_DIR%\hald__ahg_oleg_cp__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_cp HALD, gamma=1.00"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_cp.JPG

    call %IMC%  "%%f"  -gamma 1.0  %HALD_DIR%\ADD\hald__ahg_oleg_mc__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_mc HALD, gamma=1.00"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_mc.JPG

    call %IMC%  "%%f"  -gamma 1.0  %HALD_DIR%\hald__ahg_oleg_gp__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_gp HALD, gamma=1.00"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_gp.JPG

    call %IMC%  "%%f"  -gamma 0.9  %HALD_DIR%\hald__ahg_oleg_ec__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_ec HALD, gamma=0.90"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_ec.JPG

    call %IMC%  "%%f"  -gamma 0.88  %HALD_DIR%\ADD\hald__ahg_oleg_xc__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_xc HALD, gamma=0.88"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_xc.JPG

    call %IMC%  "%%f"  -gamma 0.88  %HALD_DIR%\hald__ahg_oleg_sf__16.TIF -hald-clut   %COMMON_STEREO_PARAMS%  -set comment "Processed with ahg_oleg_sf HALD, gamma=0.88"  %COMMON_OUTFILE_PARAMS%  %OUTDIR%\%%~nf__ahg_oleg_sf.JPG
    @echo off
